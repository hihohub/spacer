...
class Spacer {
    constructor() {
        ...
        this.ADDRESS = "http://3.144.127.167/welcome/scripts/Spacer10.1.js";
    ...
    SpacerCustom() { // this is a customizable function...replace this code with your own
        try {
    ...
    SpacerCustom2() { // this is a customizable function...replace this code with your own
        try {
    ...
    SpacerGetFileNameFromPath(path){
    ...
    SpacerEncodeArrows(txt,spans){//8.0
        var result = txt;
    ...
    SpacerEscapeTagsForPrint(input, allowed){ // allowed example: "<span></span><font></font>"
    ...
    SpacerEscapeTags(input, allowed){ // allowed example: "<span></span><font></font>"
        var tags = allowed.split(">");
        var full_tags = new Array();
        var partial_tags = new Array();
        for (var count = 0; count < tags.length; ++count){
    ...
    SpacerContextMenu(popup_text, timing, x, y) {
        try{
            if (document.documentElement.scrollTop){
                y += document.documentElement.scrollTop;
                x += document.documentElement.scrollLeft;
    ...
    SpacerArraySwap(index1, index2, aray){
    ...
    SpacerSearchMacro(searchterm,regular_expression,case_sensitive,exact_matches){
        try{
...
class SpacerTree {
    constructor(outer_wrapper, SPACER) {
        this.SPACER = SPACER;
    ...
    SpacerSetToolbar(toolbar_tools){
    ...
    SpacerSetType(type, alt_editor_or_not){ // 7.9.4 used by change, (new, and edit) functions...( ) = commented out
    ...
    SpacerSetTypeConditionally(type, alt_editor_or_not){ // 7.9.4 used by (load and create) functions..( ) = commented out
    ...
    SpacerSubmitEdit(html, mode, selectedspan){
        try{
    ...
    SpacerIndent(node,rightleft,indent_children){
        if (rightleft == "right"){
    ...
    SpacerLoadFromTextarea(type, preload){
        var text = "";
    ...
    SpacerEdit(type, text){
        this.LoadFromTextarea(type, text);
    }
    SpacerMouseDownSpan(evt,span){
        if (!evt){
            evt = window.event;
        }
    ...
    SpacerPrintHtml(justreturnval){
        this.PrintList(justreturnval);
    }
    SpacerPrintHtmlLines(justreturnval){
        try{
            var result;
            result = this.GetHtml();
            if (this.TYPE != null && this.TYPE != "undefined" && this.TYPE.toLowerCase() == "text"){
    ...
    SpacerGetHtmlParagraphs(){
        this.UpdateHtmlContent();
        return this.CONTENT;
    }
    SpacerGetTextForPrint(){
        this.UpdateContent();
    ...
    SpacerSwapBranches(parnt, first, second, tree, third, fourth){
    ...
    SpacerProcessTree(thisindex, nextindex, nodelist, TEMPTREE, Keys, Trimmedlines, EMPTY_FILE) {
        if (EMPTY_FILE) {
            return;
        }
    ...
    SpacerGetHtmlLines(content, lines){
        lines.length = 0;
        content = this.HtmlBody(content);
    ...
    SpacerTreeFromString(content, title, mode){
    ...
    SpacerDoNotWrapOuterElement(line){
    ...
    SpacerIsRemovableFormatTag(tag){
    ...
    SpacerDivHasKeeperNonTextTag(div){ // components not allowed unless they are listed here (and maybe also DivHasContentTag)
        var result = false;
    ...
    SpacerDivHasContentTag(div){ // don't count blanks found inside content tag or after it
        var result = false;
    ...
    SpacerDivHasSelfIndentingTag(div){
        var result = false;
    ...
    SpacerTagRequiresLineBreak(div){ // if don't wrap tables with <p> then require <br> or else two consecutive lines with tables would merge into same line
        var result = false;
    ...
    SpacerHtmlLineBreaks(HTML, nbsp){
        try{
            var html = HTML;
    ...
    SpacerSyncFromDataTree(tree, justreturnval){
        try{
            ...
            while (UL.getElementsByTagName('a').length > 0 && count < 1000){
                UL = this.SpacerRemoveAnchors(UL);
                ++count;
            }
            //var div2 = document.createElement('div');
            //div2.appendChild(UL);
            //UL = div2.children[0];
            UL = this.SpacerRemoveEmptyLists(UL);
            ...
            this.SpacerRemoveContentSpanWrappers(UL);
    ...
    SpacerCountAllChildrenOfNode(node,result){ // not working
    ...
    SpacerGetMacroIndexForNode(node){ //not working
        var datatree;
        if (SPACER.TREE){
    ...
    SpacerReplaceBranch(index, newnode){
        try{
    ...
    SpacerSetTreeHeight(treeheight){
...
class SpacerBranch {
    constructor(txt, tree, SPACER) {
        ...
        this.DISPLAY = "block";
        this.TEXT = tree.AUTO_TRIM == true ? this.SPACER.StringTrim(txt) : txt;
    ...
    // vertical top down number from root
    SpacerCountFromTop(){
        var number = -1;
    ...
    SpacerInsertBranch(node, index, update, thisindex){
        try{
    ...
    SpacerRemoveAllBranches(update){
        for (var count = 0; count < this.CHILDREN.length; ++count){
    ...
    SpacerRemoveBranch(index,update){
        try{
    ...
    SpacerShowEditBox(x, y, source, toolbar) {
        try{
            //SPACER.CloseEditBox();
            var datatree;
            if (SPACER.TREE){
